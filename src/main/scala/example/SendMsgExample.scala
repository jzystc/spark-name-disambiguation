...
object SendMsgExample {
  def main(args: Array[String]) {
    ...
    /**
      * guan jian dai ma, chuan di xiao xi, ju he xiao xi
      */
    ...
    val oldestFollower: (VertexRDD[(String, Int)]) = userGraph.aggregateMessages[(String, Int)](
      ...
      triplet=>{triplet.sendToDst((triplet.srcAttr.name, triplet.srcAttr.age))
                triplet.sendToSrc((triplet.dstAttr.name, triplet.dstAttr.age))},
      // To combine messages take the message for the older follower
      (a, b) => if (a._2 > b._2) a else b)
    oldestFollower.foreach(println(_))
    userGraph.vertices.foreach(println(_))
    userGraph.vertices.leftJoin(oldestFollower) { (id, user, optOldestFollower) =>
//    userGraph.vertices.leftOuterJoin(oldestFollower) { (id, user, optOldestFollower) =>
